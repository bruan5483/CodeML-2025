...
def filter_with_vertex_ai(input_file, output_file, rejected_file, topic, criteria, batch_size=10):
    """
    Filter JSON entries using Vertex AI Gemini for relevance
    
    Args:
        input_file: Path to input JSON file
        output_file: Path for filtered (relevant) output file
        rejected_file: Path for rejected (irrelevant) output file
        topic: Main topic to filter for
        criteria: Specific criteria for relevance
        batch_size: Number of items to evaluate per API call
    """
        ...
        try:
            ...
            for i, item in enumerate(valid_items):
                if i < len(responses):
                    ...
                    # Skip if no headline (shouldn't happen due to filtering, but just in case)
                    if not headline:
                        headline = item.get('summary', 'No content')[:60]
                    
                    display_headline = headline[:60] if headline else 'N/A'
                    
                    if 'YES' in answer:
                        relevant_items.append(item)
                        print(f"  ✓ Relevant: {display_headline}...")
                    else:
                        print(f"  ✗ Not relevant: {display_headline}...")
    ...
    rejected_items.sort(key=parse_date, reverse=True)
    
    # Write filtered data
    print(f"\nWriting relevant data to {output_file}...")
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(relevant_items, f, ensure_ascii=False, indent=2)
    
    print(f"Writing rejected data to {rejected_file}...")
    ...
    print(f"Entries with content: {len(data_with_content)}")
    print(f"Relevant entries: {len(relevant_items)} (saved to {output_file})")
    print(f"Rejected entries: {len(rejected_items)} (saved to {rejected_file})")
...
def preview_filter(input_file, topic, criteria, sample_size=20):
    ...
    for i, item in enumerate(sample, 1):
        ...
        try:
            ...
            # Show the actual headline for debugging
            if headline == 'N/A':
...
if __name__ == "__main__":
    ...
    rejected_file = "rejected_output.json"  # New file for rejected entries
    ...
    filter_with_vertex_ai(input_file, output_file, rejected_file, topic, criteria, batch_size=10)